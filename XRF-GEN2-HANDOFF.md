...
- xTool P2 硬参数对比表中，`OneLaser XRF Gen2` 表头改为更深一档的低饱和淡红，
  与淡红正文列建立清晰层级，同时不增加红色分隔线。
- 手机与平板的功能章节导航改为固定在全局 Journey 导航下方：
  手机使用 `top: 106px`，561–760 px 使用 `top: 124px`，并同步调整章节锚点偏移。
- Hero 配件缩略图新增固定裁切窗；移除 CSS 内边距和 letterboxing，并按四张官方
