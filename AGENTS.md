...
- Keep the XRF Gen2 at-a-glance module as one static long-form image with no internal interaction. Place it immediately before the FOX proof module, let it span almost the full viewport with 24 px desktop / 12 px mobile gutters, and use `#141414` behind it. Until the dedicated mobile asset arrives, contain the desktop image without cropping.
